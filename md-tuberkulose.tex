...
Die Parameter des SIR Modells sind $\mu$ die Sterberate, $\beta$ die Infektionsrate und $\gamma$ die Erholungsrate. Die Sterberate ist die generelle Sterberate in der Bevölkerung. Für ein realeres, kontinuierliches Modell mit Zyklen, bräuchte dieses Modell auch einen entsprechenden Geburtenratenparameter. Das vorliegende einfache $SIR$ Modell ist ein nicht-lineares Differentialgleichungssystem erster Ordnung, durch $\frac{I}{N}S$, da $S$ nicht mit einer Konstanten multipliziert wird. Dadurch sind keine analytischen Lösungen möglich. Das SIR Modell kann leicht durch latent infizierte (Exposed) zum SEIR Modell erweitert werden. Das DGLS \ref{eq:dgls-seir} bekommt eine zusätzliche Klasse $E$ mit dem korrespondierenden Zunahmeparameter $\alpha$, wobei $N = S + E + I + R$ gilt.
...
Es fällt auf das diese Differentialgleichssysteme jedes Compartment mit einem korrespondierenden Zunahmeparameter belegt, die Übergangswahrscheinlichkeiten darstellen. Sollte es z.B. möglich sein direkt von $S$ zu $I$ zu laufen, kann man einen entsprechenden Zunahmeparameter einfügen.
